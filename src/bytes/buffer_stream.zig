...
pub fn BufferStream(comptime T: type) type {
    return struct {
        const Self = @This();
        ...
        pub const ReadError = error{} || T.Error;
        pub const WriteError = error{} || T.Error;
        ...
        pub fn getEndPos(self: *Self) GetSeekPosError!u64 {
            if (self.buffer.rawLength() == 0) return GetSeekPosError.IsEmpty;
            return self.buffer.rawLength();
        }
...
test "output" {
    ...
    var buff = Buffer.init(arena.allocator());
    defer buff.deinit();
