...
const zarchive = @import("archive.zig");
    ...
    return struct {
        ...
        pub fn deccompressWithFilters(self: *Self, filters: std.ArrayList([]const u8), receiver: anytype) !void {
            if (self.archive.local_file_entries.items.len == 0) {
                self.cleanAndReplaceZipArchive(try zarchive.parse(self.allocator, self.source, .{}));
            }
            ...
            for (self.archive.central_diectory_headers.items) |cdheader| {
                ...
                const lfentry = try zarchive.readLocalFileEntry(self.allocator, cdheader, seekableStream, reader, .{});
